...
With `LLM_PROVIDER=openrouter` every model request — the weekly report and the market-rate calls — is additionally pinned to zero-data-retention endpoints (`provider.zdr`, `data_collection: "deny"`, `src/openrouter.ts`): the model provider keeps neither the prompt nor the completion and cannot train on them. This is hardcoded, not a setting — if no ZDR endpoint can serve the model, the request fails instead of falling back to one that retains data. ZDR does not extend to OpenRouter's web search tool: its queries reach the search engine under that engine's own policy, and they are built from the extracted role/seniority/stack/region only (see below). OpenRouter itself does not store prompts unless prompt logging is enabled in the account's privacy settings; keep it off.

Structured output is asked for, but not at the cost of the run. A request carrying `response_format: json_schema` is routed by OpenRouter only to endpoints that implement it — for the model used here that is a single provider, and `require_parameters: false` does not widen it — so when that provider was rate-limited upstream the whole weekly run died with it. The report call therefore makes a second attempt without structured output at all: the schema moves into the instructions, the rest of the ZDR pool becomes eligible, and the answer is parsed, reshaped and validated in `src/llm.ts` (`normalizeLlmAnswer`, fenced-JSON unwrapping) before anything is built from it. Retention is never what gets traded away — both attempts are ZDR-only.

The market-rate lookup has no such fallback: it stays on structured output, and a failure there is not fatal (cached rates are reused, or the salary section is left out).
