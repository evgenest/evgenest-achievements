...
const privateRepo = makeRepo({
  ...
  isPrivate: true,
  commits: [makeCommit({ message: "feat: acquisition pricing model", url: "https://github.com/octocat/secret-thing/commit/a" })],
});

const week = () =>
  makeWeek({
    repos: [makeRepo(), privateRepo],
    prs: [makePr(), makePr({ title: "Secret PR", repo: "octocat/secret-thing", isPrivate: true })],
    issues: [makeIssue({ title: "Secret issue", repo: "octocat/secret-thing", isPrivate: true })],
  });
...
describe("applyPrivacy", () => {
  ...
  it("redact: keeps metrics but strips names, messages and links", () => {
    ...
    expect(redacted?.commits[0]).toMatchObject({ message: "", url: "" });
  ...
  it("redact: no private name or title survives serialization", () => {
  ...
  it("redact: public activity keeps its names and links", () => {
    const result = applyPrivacy(week(), "redact");
