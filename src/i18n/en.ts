...
Rules:
- projectSummaries: an ARRAY of objects shaped {"repo": "...", "summary": "..."} — one object per repository in the data. NOT a dictionary keyed by repository name. The repo field must be the full repository name exactly as in the data. The summary field is 2-5 sentences in plain language about what was actually accomplished (business logic, value), not a retelling of commit messages. The data carries full commit messages (headline + body, in week.commits) and PR/issue descriptions (body) — use them to explain the substance and motivation of the changes; [secret] marks a scrubbed secret, never mention it. If a repository has no commit messages (a private project with hidden details), keep the summary neutral and metrics-based.
